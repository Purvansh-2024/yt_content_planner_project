...
def build_html_report(plan: dict, schedule_df: pd.DataFrame, niche: str) -> str:
    ideas = plan.get("video_ideas", [])
    channel_summary = plan.get("channel_summary", "")
    ...
    for i, idea in enumerate(ideas, start=1):
        tags = ", ".join(idea.get("seo_tags", []))
        idea_cards += f"""
        <div class="idea-card">
            <h3>{i}. {esc(idea.get('seo_title', idea.get('video_idea', 'Untitled')))}</h3>
            <p><span class="label">Concept:</span> {esc(idea.get('video_idea', ''))}</p>
            <p><span class="label">Format:</span> {esc(idea.get('video_format', ''))}</p>
            <p><span class="label">SEO Tags:</span> {esc(tags)}</p>
            <p><span class="label">Thumbnail Concept:</span> {esc(idea.get('thumbnail_concept', ''))}</p>
        </div>"""
    ...
    for _, row in schedule_df.iterrows():
        schedule_rows += f"""
        <tr>
            <td>{esc(row['Upload Date'])}</td>
            <td>{esc(row['Video Title'])}</td>
...
<head>
<meta charset="UTF-8">
<title>YouTube Content Plan — {esc(niche)}</title>
<style>
    body {{
        background-color: #0f0f0f;
        color: #f1f1f1;
    ...
    .play-mark svg {{ width: 16px; height: 16px; }}
    h1 {{ margin: 0; font-size: 1.6rem; }}
    .subtitle {{ color: #aaaaaa; margin: 4px 0 0; font-size: 0.95rem; }}
    h2 {{ color: #ff4d4d; font-size: 1.2rem; margin-top: 2.2rem; }}
    .summary-box {{
        background-color: #212121;
        border: 1px solid #303030;
        border-radius: 10px;
    ...
    .idea-card h3 {{ margin: 0 0 0.6rem; font-size: 1.05rem; color: #ffffff; }}
    .idea-card p {{ margin: 0.3rem 0; font-size: 0.92rem; line-height: 1.5; }}
    .label {{ color: #ff8a8a; font-weight: 600; }}
    table {{
        width: 100%;
        border-collapse: collapse;
        font-size: 0.92rem;
    ...
    tr:nth-child(even) td {{ background-color: #1c1c1c; }}
    .footer {{
        margin-top: 2.5rem;
        color: #666;
        font-size: 0.8rem;
...
<body>
    <div class="header">
        ...
        <div>
            <h1>AI YouTube Content Planner</h1>
            <p class="subtitle">Content plan for: {esc(niche)}</p>
        </div>
    </div>

    <h2>Strategy Summary</h2>
    <div class="summary-box"><p>{esc(channel_summary)}</p></div>

    <h2>Video Ideas</h2>
    {idea_cards}

    <h2>Suggested Upload Schedule</h2>
    <table>
        <tr><th>Upload Date</th><th>Video Title</th><th>Format</th></tr>
        {schedule_rows}
    </table>

    <p class="footer">Generated by AI YouTube Content Planner</p>
</body>
</html>"""
...
if "plan" in st.session_state:
    ...
    with col3:
        html_report = build_html_report(plan, schedule_df, niche)
        st.download_button(
            "Download Report (HTML)",
            data=html_report,
            file_name="youtube_content_plan.html",
            mime="text/html",
            use_container_width=True,
        )
else:
